...
def write_markdown(result: AnalysisResult, path: Path) -> None:
    ...
    # チーム別は（未設定）を除外（チーム未設定は部署も異なる異質な集合のため）
    team_block = _group_summary_md(users, s, "team", "チーム別サマリ", include_unset=False)
    team_note = (
        "\n- チーム別サマリはチーム未設定のユーザを除外しているため、"
        "縦合計は組織全体と一致しません。" if team_block else "")
...
def write_html(result: AnalysisResult, path: Path) -> None:
    ...
    for col, heading in (("department", "部署別サマリ"), ("team", "チーム別サマリ")):
        # チーム別は（未設定）を除外（チーム未設定は部署も異なる異質な集合のため）
        rows = _group_summary_rows(result.users, result.summary, col, include_unset=(col != "team"))
        if not rows:
            continue
        for t in rows:
