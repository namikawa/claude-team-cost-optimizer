...
業務情報を検出したら終了コード 1 を返し、検出語と一致箇所を表示する。検査の対象語は
`input/` と `reports/` から機械的に集める（全組織の組織名・メールアドレス・人名・部署名）。

「すでに公開されている内容」に現れる語は検出しない。合成サンプルデータ（`examples/`）の
人名が実在の姓と偶然一致していても、その文字列は既にリポジトリにあるため新たな開示には
あたらない、という考え方。この基準に使う範囲は README・config.yaml・`examples/`・`src/`・
`tests/`・`.claude/` で、`input/`・`reports/`・CLAUDE.md（いずれも gitignore 対象）は
含めない。別のディレクトリを基準にしたい場合は `--repo-root` を指定する。
