...
- 現在のリリーススコープ: v1.1.1（判定不変のPATCH）までリリース済み。Milestone Aは
  ...
  - v1.1.2（PATCH）: Fable 5.1 / Mythos の単価パターンとモデル別のキャッシュ読取倍率
    （`model_prices.patterns[].cache_read`）への対応、CHANGELOGの`[未リリース]`に積んで
  ...
  - v1.3.0（MINOR）: Track 9 の Phase 1（Step 43〜48）。複数workspaceの組織だけ成果物の形が
    変わり、従来レイアウトの組織の判定・成果物は不変（golden で固定）。Step 49（切替の観測）は
    両workspaceの週次エクスポートが揃ってから別の版で出す
...
| 38 | github-summary.csv | `完了` | 2026-09-04 |
...
- Step 36〜37 の純粋関数を分析経路へつなぎ、参考値の成果物
  `github-summary-YYYYMM-<組織名>.csv`を書くようにした。対象は`organizations.<組織名>.
  github_org`を設定した組織だけで、設定の無い組織は GitHub 関連の処理と通知の一切から
  外れる。速報モードでは書かず1行知らせる。V1 の成果物と組織横断サマリには触れない
- repository 除外の材料（archived / fork / template を除いた一覧）は、収集時に月キャッシュ
  `prs-YYYY-MM.json`の任意キー`repositories`へ保存する形にした（Step 36 からの申し送りの
  決定）。分析は gh もネットワークも呼ばずキャッシュだけで参考値を出すので、同じ
  キャッシュからは常に同じ内容になり、オフラインでも完走する。形式の版は上げていない——
  任意のキーの追加は読み側の互換を壊さないため、一覧を保存する前に作られたキャッシュも
  そのまま読める（`repositories=None`）。その場合は参考値を書かず、収集の再実行を案内する
- `PrCache.repositories`は「同じ Organization の完全な一覧」だけを受ける。
  `collect_merged_prs`は`repos`をキーワード必須の引数にし、型違い・Organization 不一致・
  不完全な一覧では何も書かずに中止する。CLI の collect は PR の検索より先に一覧を取り、
  完全な一覧を得られなければ検索へ進まずキャッシュも書かない（部分的な一覧で集計すると、
  一覧に載らなかった repository の PR が「対象外」へ流れ、参考指標が黙って小さく出る）
- CSV は1ファイルで`scope`列が行の単位を表す。個人行はメールアドレス昇順のままで、
  件数の多い順へ並べ替えない（設計書 §15.6 の「個人ランキングを作らない」）。組織全体行は
  末尾1行で、集計から外した分の内訳6列はこの行だけが値を持つ。lead time は小数1桁で、
  PR が0件の行は3列とも空欄（0 で埋めると即時 merge と区別できない）
- 「参考値である」ことの明示は CSV の定数列ではなく、`docs/reference.md`の列説明と実行時の
  出力の見出しで担う。全行同じ値の列を増やしても読み手の判断は変わらないため
- repository 名・GitHub の Organization 名・対応表に無い作成者の login は成果物に書かない
  （要約がこれらを持たないので、出る余地が無いことをテストで固定した）
- テスト: 2113 passed（+68件。キャッシュの往復とバイト一致・一覧を持たないキャッシュ・
  壊れた一覧18通り・値オブジェクトの不変条件・収集の拒否・CSV の形と表記20件・結線10件・
  collect の2件）、ruff 緑、`check-text --diff`は0件
- 残っている成果物への通知は、有効にした組織で書けなかった実行だけに出す。`organizations`
  から外した組織では通知もしない（設計書 §15.1）。毎月同じ通知が出続けると、対処すべき
  警告がその中に埋もれるため
- 外部レビュー（codex 1巡目・受け入れ基準9点に範囲固定）: 8点は指摘なし。mid 1件
  （残っている成果物の通知がゲートの外にあり、設定に無い組織でも出る）を採用し、通知を
  ゲートの内側へ移した。両方向の挙動をテストで固定した（有効な組織で書けなければ出る・
  設定から外れていれば出ない）
- 実データ確認: GitHub 分析を有効にした2組織の 2026-08 で、収集を再実行して一覧を保存し、
  分析を scratch 出力で流した。キャッシュの差分は一覧の追加だけで PR の内容は不変。
  区分の検算（個人行の合計 + 内訳4列 = 全件数）と組織全体の母数の対応が全件成立し、
  件数と lead time の3点は Step 36・37 の実データ確認の記録と一致した。2回流してバイト
  一致。V1 の成果物4種（details / dashboard / recommendations / usage-summary）は
  既存のレポートとバイト一致で、判定と成果物に影響しないことを実データでも確認した。
  repository 名・Organization 名・対応表外の login の露出は0件
...
- 公式の料金ページで2点を確認した。①Sonnet 5 の $2/$10 は導入価格（2026-08-31まで）として
  発表されたが、そのまま正式価格になり $3/$15 への値上げは行われない ②Fable 5.1 と
  Mythos 5.1 のキャッシュ読取（cache hits and refreshes）だけ基本入力単価の0.025倍で、
  他モデルは従来どおり0.1倍。書込（5分1.25倍・1時間2.0倍）は全モデル共通
- ①により予定していた単価改定は不要になった。`default-config.yaml`のsonnet-5の値は
  変更せず、コメントを「導入価格がそのまま正式価格になった」旨へ書き換えた（誤った改定を
  後から入れないため）
- ②は`cache_multipliers.read`が全モデル共通の1値だったため、Fable 5.1 を使うユーザの
  キャッシュ読取が4倍に過大計上される。実データではキャッシュ読取が入力トークンの
  大半を占めるため、需要そのものが数倍膨らむ。公式ではFable 5.1が現行でFable 5がlegacy
  なので、9月のデータから入りうる
- 単価パターン（`model_prices.patterns[]`）に任意キー`cache_read`（入力単価に対する倍率）を
  足し、省略したパターンは従来どおり`cache_multipliers.read`へ落ちる形にした。書込の倍率は
  per-modelにしていない（公式で全モデル同じ）
- パターンは`fable-5-1`・`mythos-5-1`（ともに`cache_read: 0.025`）と`mythos`（$10/$50）を
  追加した。「特例は汎用パターンより上」の既存規則どおり順序で効かせ、無印の`fable`は
  0.1倍のまま。Mythos は限定提供だが料金表に載っているので、出現時に既定の$5/$25へ
  落ちないようにする
- 倍率の解決は`pricing.cache_read_multiplier_for_model()`を新設して行い、行ごとに引く
  （`add_computed_cost`が`df["model"].map`で解決する）。`price_for_model`の戻り値の形は
  変えていない。単価と倍率が同じパターンを引くよう、照合の規則は`_matching_pattern()`に
  1箇所化した
- config検証は`cache_read`を「書かれていれば正の有限な数値」とした。0以下だとキャッシュ
  読取が無料・値引きとして計上され、非有限だと需要がNaNか無限大になる。`model_prices.default`
  側には書けない（既定のキー集合に無く、上書きのマージ検査がフルパス付きでエラーにする）
- 追加クレジット上限の字句規則（Step 17で厳格化。全角数字・円記号・桁区切りの`_`・非有限値・
  負値を不明+警告へ倒す）をdocs（`reference.md`の追加クレジットの節）へ書き、CHANGELOGの
  「変更」に1行載せた。文言はコード（`ingest.is_number_text`・`parse_credit_limit`）で
  確認した規則に合わせている
- 外部レビューの採用①: 単価パターンの要素に書けるキーを`{match, input, output, cache_read}`へ
  閉じた。リストは丸ごと置換されるため`_merge_override`のキー検査が要素の中へ届かず、
  `cache_raed`のような綴り違いが通って倍率が黙って既定へ落ちていた（再現済み）。検査は必須キーの
  検査より前に置き、必須キーの綴りを誤った場合もその綴りを名指しする
- 外部レビューの採用②: `reference.md`の「無制限」の箇条書きを`_UNLIMITED_TOKENS`と完全一致
  させた（`inf`・`∞`も語彙に含まれる）。挙動は変えていない
- 1615 passed（新規15件: モデル別倍率とフォールバック・単価パターンの順序・未マッチ判定・
  config検証）・ruff緑・golden不変（examplesの合成データに fable-5-1 / mythos は無く、
  無印`claude-fable-5`のパターン一致も倍率も変わらないため出力は1バイトも動かない）
