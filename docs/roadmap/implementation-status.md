...
- 次のタスク: sonnet-5 の単価改定（導入価格 $2/$10 は 2026-08-31 まで。以降は $3/$15 へ
  `default-config.yaml` を更新する。PATCHリリースになる）
- 現在のリリーススコープ: v1.1.0（= Track 2。Step 6〜8。8F・8E・8Dを含む）をリリース済みで、
  設計書のMilestone Aはここで完了した。次のリリースのスコープは未定
