...
- 列が無い場合だけでなくセルが欠けている場合も同じ扱いにする。`product`名が空で分類できない
  行、`requests`が欠けた行があるユーザでは、その行の内容しだいで変わる特徴量を欠損にする。
  伝播はユーザ単位に閉じ、欠損の無いユーザには波及しない
- 例外は「1行でも観測した」という存在の主張。`prohibited_observed`と`supplementary_high`は
  真なら分類できない行が何であっても覆らないので残し、偽になる場合だけ欠損にする。ただし
  policyにその分類の名前が1つも無いとき（既定の`prohibited`）は、空の行が何であっても
  一致しようがないため偽が確定する
