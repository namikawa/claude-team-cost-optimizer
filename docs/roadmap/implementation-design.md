...
- 表示する数値を増やさない。既存の数値からその場で導ける表現（判定の内訳・追加クレジットの
  状態の帯・月次推移のバー・タブの件数バッジ）はデザインの一部として認めるが、新しい集計・
  新しい入力は足さない。件数バッジは**そのタブに実際に並んでいる行数**と一致させること
  （描画されていない軸を数えると、部署が無くチームだけある組織で 0 と出る）
- 既存sectionの中身は移動のみとする。ただし「前月からの変化」のユーザ表記だけは例外で、
  完全なメールアドレスからローカル部 + `title`へ変える（2026-08-17 ユーザ裁定）。デザインの
  意図であり、推奨一覧・棒グラフなど隣接する表示と体裁が揃うため。外部レビューは印刷時に
  `title`が読めない点を指摘したが、同一ドメインの組織では取り違えが起きないため許容する
- Lightテーマでも文字として使う色が背景に対して4.5:1以上であること（大きい文字は3:1）。
  デザインのトークンをそのまま採るとLightだけが基準を下回る（Dark側は満たす）
...
- **検索**: ユーザ名＋メールアドレスの部分一致（大文字小文字を無視）。入力欄は幅200px・
  `padding: 7px 11px`・背景`--surface-2`・`1px solid --line`・`border-radius: 8px`、
  フォーカス時に`border-color: --accent`。絞り込み中はカードの件数表示にその旨を添える
- **判定フィルタ**: 推奨一覧のみ。選択肢は実データに現れた判定から作る（固定リストにしない。
  出ない判定を選べると空表になる）
- **行数の折りたたみ**: 表は初期15行、バーは初期20本。超過分は「残りN件（名）を表示」で
  全件展開する。再折りたたみは実装しない
...
- 8D-1と同じ出力不変条件を満たす
- JSが無効な環境では、折りたたみ・ソート・検索・高さ変更のUIが現れず、全行が表示された
  ままになる。内容が欠けたり操作できないUIが残ったりしない
