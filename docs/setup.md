...
すでに手元にある場合はこのステップを飛ばす。以降のコマンドはすべてリポジトリのルートで
実行する（サブディレクトリで実行すると設定ファイルと入力ディレクトリの解決に失敗する）。

Windows で、以前に clone したものを使う場合はワークツリーを取り直す。テストは生成物と
同梱の見本をバイト単位で比べるため、見本の改行が CRLF のままだとステップ 5 で失敗する。
...
`git reset --hard` は未コミットの変更を捨てるので、残したい変更があれば先に退避する。
新しく clone した場合はリポジトリの `.gitattributes` が改行を LF に揃えるため、この
操作は要らない。
