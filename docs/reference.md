...
込み枠の実測・感度分析を集めた機械生成の詳細資料、dashboard.html は同じ数値をタブに
分けて見せる共有用の自己完結 HTML。

以下のセクション名は、断りがなければ details.md と dashboard.html の両方に出るものを指す。
片方にしか出ないものもあり、「込み枠の実測（E = API換算需要 − 実課金）」と「感度分析」は
details.md だけ、「Codeと他プロダクトの需要（API換算）」は dashboard.html だけに出る。
...
- 行の折りたたみ — 表は15行、棒グラフは20本まで表示し、「残り N 名を表示」で全件に切り替わる
  （一度広げた一覧は畳み直さない）
- 高さの変更 — 表のスクロール領域は、その直下のバーをドラッグして縦の高さを変えられる
  （動かしても見え方が変わらない短い表にはバーが出ない）

JavaScript が無効な環境でも内容はすべて読める。その場合は上記の操作用の UI が現れず、
4つのタブの中身が見出し付きで縦に積まれ、折りたたみもされないので全行が表示される
（テーマの切替ボタンも出ないが、明暗は OS の設定に追従する）。
...
## Codeと他プロダクトの需要（API換算）（dashboard.html のみ）
...
上のセクションと usage-summary.csv の `supplementary_high` / `prohibited_observed` 列は、
...
## usage-summary.csv の列

ユーザ単位の product 利用特徴量。email + 8つの特徴量の計9列で、行は対象月のスペンドに明細の
あるユーザ（メールアドレス昇順）。利用ゼロのメンバーは行を持たないため recommendations.csv とは
